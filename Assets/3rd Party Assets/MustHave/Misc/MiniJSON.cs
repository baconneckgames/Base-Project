...
/// <summary>
/// Mini JSON class based off P31 minijson.
/// </summary>
public class MiniJSON
{
	...
	public static object jsonDecodeDict (string json)
	{
		// save the string for debug information
		MiniJSON.lastDecode = json;
		...
		if (json != null) {
			...
			int index = 0;
			bool success = true;
			object value = MiniJSON.parseDictValue (charArray, ref index, ref success);
	...
	public static string jsonEncodeListDict (List<Dictionary<string, string>> json)
	{
		var builder = new StringBuilder (BUILDER_CAPACITY);
		var success = MiniJSON.serializeDictionaryList (json, builder);
	...
	protected static Dictionary<string, string> parseDictObject (char[] json, ref int index)
	{
		//Hashtable table = new Hashtable ();
		...
		while (!done) {
			...
			if (token == MiniJSON.TOKEN_NONE) {
				...
				// value
				bool success = true;
				object value = parseDictValue (json, ref index, ref success);
				if (!success)
					return null;
	...
	protected static List<Dictionary<string, string>> parseDictArray (char[] json, ref int index)
	{
		//ArrayList array = new ArrayList ();
		...
		while (!done) {
			...
			if (token == MiniJSON.TOKEN_NONE) {
				...
				array.Add ((Dictionary<string, string>)value);
			}
		}
	...
	/*protected static List<Dictionary<string, string>> parseList (char[] json, ref int index)
	{
		List<Dictionary<string, string>> array = new List<Dictionary<string, string>>();
		...
		while (!done) {
			...
			if (token == MiniJSON.TOKEN_NONE) {
				...
				bool success = true;
				Dictionary<string, string> value = parseValueLists (json, ref index, ref success);
				if (!success)
					return null;
	...
	/*protected static Dictionary<string, string> parseValueLists (char[] json, ref int index, ref bool success)
	{
	...
	protected static object parseDictValue (char[] json, ref int index, ref bool success)
	{
		switch (lookAhead (json, index)) {
		case MiniJSON.TOKEN_STRING:
			return parseString (json, ref index);
		case MiniJSON.TOKEN_NUMBER:
			return parseNumber (json, ref index);
		case MiniJSON.TOKEN_CURLY_OPEN:
			return parseDictObject (json, ref index);
		case MiniJSON.TOKEN_SQUARED_OPEN:
			return parseDictArray (json, ref index);
		case MiniJSON.TOKEN_TRUE:
			nextToken (json, ref index);
			return Boolean.Parse ("TRUE");
		case MiniJSON.TOKEN_FALSE:
...
*/
	...
	protected static bool serializeDictionaryNested( Dictionary<string, Dictionary<string, string>> dict, StringBuilder builder)
	{
		builder.Append ("{");
		...
		foreach (var kv in dict) {
			...
			builder.Append (":");
			serializeDictionary (kv.Value, builder);
	...
	protected static bool serializeDictionaryList(List<Dictionary<string, string>> anArray, StringBuilder builder)
	{
		builder.Append ("[");
		
		bool first = true;
		for (int i = 0; i < anArray.Count; i++) {
			Dictionary<string, string> value = anArray[i];
	...
	protected static bool serializeValue( object value, StringBuilder builder )
	{
		...
		else if( value is Dictionary<string, Dictionary<string, string>>)
		{
			serializeDictionaryNested( (Dictionary<string, Dictionary<string, string>>)value, builder);
		}
		else if( value is List<Dictionary<string, string>>)
		{
			serializeDictionaryList((List<Dictionary<string, string>>)value, builder);
		}
...
public static class MiniJsonExtensions
{
	...
	public static string toJson( this Dictionary<string, Dictionary<string, string>> obj)
	{
		return MiniJSON.jsonEncode( obj );
	}
	
	public static string toJson( this List<Dictionary<string, string>> obj)
	{
		return MiniJSON.jsonEncodeListDict( obj);
	}
