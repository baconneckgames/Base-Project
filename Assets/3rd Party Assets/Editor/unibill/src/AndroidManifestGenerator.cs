...
public class AndroidManifestGenerator {
    ...
    public static void mergeManifest() {
		...
		string xml = new UnityResourceLoader ().openTextFile ("unibillInventory").ReadToEnd ();
        UnibillConfiguration config = new UnibillConfiguration(xml, new UnibillXmlParser(new Mono.Xml.SmallXmlParser(), new UnityResourceLoader()), new UnityLogger());
